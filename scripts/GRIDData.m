...
dataDistance = 'GRID/GRID-toolkit/output/distance_matrix.csv';
...
J = size(dni, 1);                                                           % Update J to reflect the chosen grid                                                                      
...
griddataname = 'GRID/GRID-toolkit/output/grid-data.csv';                                            %These data contain within-commuting as weighted average
griddata = csvread(griddataname, 1, 1);
L = sum(griddata(:,3));
...
save('data/output/DATAusingGRID')

display('<<<<<<<<<<<<<<< Data compilation completed >>>>>>>>>>>>>>>')

% Inspection of results
RESULT = GRIDMAPIT('GRID/GRID-toolkit/output/grid-data',L_n,'Employment','figs','GRID_MAP_emp') 
RESULT = GRIDMAPIT('GRID/GRID-toolkit/output/grid-data',R_n,'Population','figs','GRID_MAP_pop') 
RESULT = GRIDMAPIT('GRID/GRID-toolkit/output/grid-data',A_n,'Productivity','figs','GRID_MAP_prod') 
