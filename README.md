...
A complementary **GRID version** of this toolkit has been developed to facilitate the execution of counterfactuals in arbitrary real-world settings. The GRID implementation seamlessly integrates with the **[GRID-toolkit](https://github.com/Ahlfeldt/GRID-toolkit)** and the **[TTMATRIX-toolkit](https://github.com/Ahlfeldt/TTMATRIX-toolkit)**, allowing users to generate compatible input data such as grids, population and employment measures, and travel time matrices. In particular, the workflow becomes straightforward when feeding data from the **AABPL-toolkit** into the **GRID-toolkit**, which then provides ready-to-use inputs for the **GRID version** of the present toolkit. It is recommended that readers first complete the reading of this README file and familiarize themselves with the baseline didactic counterfactuals for the German setting before proceeding to the GRID version, which is described in a separate README file.
