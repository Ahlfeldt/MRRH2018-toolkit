...
A complementary **GRID version** of this toolkit has been developed to facilitate the execution of counterfactuals in arbitrary real-world settings. The GRID implementation seamlessly integrates with the **[GRID-toolkit](https://github.com/Ahlfeldt/GRID-toolkit)** and the **[TTMATRIX-toolkit](https://github.com/Ahlfeldt/TTMATRIX-toolkit)**, allowing users to generate compatible input data such as grids, population and employment measures, and travel time matrices. In particular, the workflow becomes straightforward when feeding data from the  **[GRID-toolkit](https://github.com/Ahlfeldt/GRID-toolkit)** into the **[AABPL-toolkit](https://github.com/Ahlfeldt/AABPL-toolkit)**, which then provides ready-to-use inputs for the **GRID version** of the present toolkit. It is recommended that readers first complete the reading of this README file and familiarize themselves with the baseline didactic counterfactuals for the German setting before proceeding to the GRID version, which is described in a separate For further detail, go to [README](https://github.com/Ahlfeldt/MRRH2018-toolkit/tree/main/GRID) file.
...
| --- | --- | --- | --- |
| `data/input` | | Folder containing required data inputs to execute this toolkit | -|
| `data/input` | `commuting_wide.csv` | CSV file containing bilateral commuting flows | -|
| `data/input` | `CountyArea.csv` | CSV file containing county geographic area in sq. km |- |
| `data/input` | `CountyBorderDist.csv` | CSV file containing distance from the inner-German border n km | -|
| `data/input` | `distance_matrix.csv` |  CSV file containing bilateral distances between counties |-|
| `data/input` | `house_prices.csv` |  CSV file containing county house prices |-|
| `data/input` | `labor_tidy.csv` |  CSV file containing labour market outcomes |-|
| `data/input` | `roundtrip_time_base.csv` |  CSV file containing bilateral commuting times |-|
| `data/output` | | Folder containing data files generated by the scripts in this toolkit. | Will be populated while you execute the MATLAB programs |
| `figs` | | Folder containing figures and maps  |Will be populated while you execute the MATLAB programs |
| `GRID` | | Folder containing inputs for the GRID version of the toolkit |
